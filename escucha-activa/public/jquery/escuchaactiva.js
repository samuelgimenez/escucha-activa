...
/**
 * Esta función rellena en una tabla con los datos obtenidos de http://opendata.aragon.es/socialdata/
 */
		...
		else
		{
		 ...
		 if(typeof miObj === 'undefined' ){
				...
				{console.log(miObj);
				 	...
				 	//var numPaginas= miObj.total_pages; //Ya no hay esto en la api 10/11/2014
					//Límite de páginas a ver
					switch(miObj.results[0].type){
						case "event":
							numPaginas= 117;
							break;
						case "venue":
						...
						case "wiki":
							numPaginas= 81;
							break;
						default: //text, picture, video, all
							numPaginas= 1000;
						}
					...
					{	
			   		...
			   		switch(miObj.results[i].type){
							case "video":
								...
								break;
							case "picture":
								$("#tablaDatos").append("<tr><td></td><td class='i_i'><img src="+miObj.results[i].thumbnail+" alt='"+miObj.results[i].title+"' class='ImageBorder'/></td></tr>");
								$("#tablaDatos").append("<tr style='height:5px;'><td>");
								break;
							default: //text, event, venue, code, wiki
								$("#tablaDatos").append("<tr style='height:5px;'><td>");
						}
						//Ahora añadimos la línea con el enlace  y los botones de compartir
						$("#tablaDatos").append("<tr><td></td><td class='i_i abrir'><a class='sizeData' href="+miObj.results[i].url+" title='Abrir' acronym='Abrir recursos en origen'>Abrir</a></td><td class='d_d sizeData compartirrs'><span style='margin:0 10px 0 0;'>Compartir</span> <a href=https://www.facebook.com/sharer/sharer.php?u=="+miObj.results[i].url+"  title='Compartir en Facebook'><img src='../escucha-activa/public/i/09-ICON-Facebook.png' width='15px' height='15px' alt='Facebook'/></a><a href=https://twitter.com/intent/tweet?url="+miObj.results[i].url+" title='Compartir en Twitter'><img src='../escucha-activa/public/i/10-ICON-Twitter.png' alt='Twitter' width='15px' height='15px'/><a href=https://plus.google.com/share?url="+miObj.results[i].url+"  title='Compartir en Google+'><img src='../escucha-activa/public/i/12-ICON-Google-Plus.png' style='padding-right:5px;'  width='15px' height='15px' alt='Googleplus'/></a></td></tr>");
						$("#tablaDatos").append("<tr style='height:15px;'><td>");
						$("#tablaDatos").append("<tr class='separadorpaginas'><td colspan='2'></td></tr>");
...
function getEmbeddedPlayer(url, height, width){
	...
	if( youtubeUrl ){
		...
		output = '<iframe src="//player.vimeo.com/video/'+vimeoUrl[0].substring(vimeoUrl[0].lastIndexOf("/")+1)+'" width="'+width+'" height="'+height+'" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
...
/**
 * Esta función vacia la pantalla, inicializa el mapa para luego pintarlo según el parametro dado
 * El filtro con el que rellenaremos los datos, son los del segundo combobox que que tiene como nombre de
 */
function montarMapa(filtro)
{
	$("#datawindow").empty();
	$("#paginator").empty();
	$("#mapaDinamico").remove();
	inicializaMapa(filtro);
}


/**
 * Esta función inicializa el mapa para luego pintarlo según el filtro dado
 * El filtro con el que rellenaremos los datos, son los del segundo combobox que que tiene como nombre "de", la procedencia
 */
function inicializaMapa(filtro)
{
  $("#datos_terceros").hide();
  $("#miMapa").hide();
  var mapProp = {
  ...
  var fechaString = convertirFecha(fechaInicio) ;
	...
	//alert('El mapa saca los datos en '+cadenaUrl);
...
function cargaMarcadores()
{
	...
	{	 textoInfo.length = 0;
			...
			var contador = 0;
			var marcadores;
			if (miObj.on_this_page>0){
			//alert('Hay '+ miObj.results.length +' marcadores');
				for (i = 0; i < miObj.results.length; ++i) 
				{
				...
				//alert('No tiene marcadores');
				$("#datawindow").append("No tiene marcadores");
			}
			if (contador > 0)
			{
...
/**
 * Conversor de fecha con formato Wed Aug 27 2014 16:42:33 GMT+0200 a formato 27/08/2014
 * @param {Date} fecha  
 */
